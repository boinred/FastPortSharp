...
[TestClass]
public sealed class FastPortTestLoadValidationTests
{
    [TestMethod]
    public void LoadValidationOptions_TryParse_UsesDefaults()
    {
        ...
        Assert.AreEqual("FastPortTestLoadRunner", options.RunnerProject);
    ...
    [TestMethod]
    public void LoadValidationOptions_TryParse_UsesTunedAdaptiveDefaults()
    {
    ...
    public void LoadRunnerCommandBuilder_BuildsStageCommand()
    {
        var options = new LoadValidationOptions(
            ...
            OutputDirectory: "out",
            StageId: null,
            RunnerProject: "FastPortTestLoadRunner",
        ...
        CollectionAssert.Contains(command.Arguments.ToArray(), "--pacing-initial-window");
        ...
        CollectionAssert.Contains(command.Arguments.ToArray(), "--pacing-rtt-target-ms");
        CollectionAssert.Contains(command.Arguments.ToArray(), "1000");
        CollectionAssert.Contains(command.Arguments.ToArray(), "--pacing-rtt-high-ms");
        CollectionAssert.Contains(command.Arguments.ToArray(), "2000");
        CollectionAssert.Contains(command.Arguments.ToArray(), "--pacing-increase-every");
        CollectionAssert.Contains(command.Arguments.ToArray(), "3");
    }
    ...
    [TestMethod]
    public void LoadValidationEvaluator_FailsFinalDisconnects()
    {
        LoadValidationStage stage = CreateStage(targetSessions: 10);
        JsonlReadResult readResult = new(
            [
                CreateSample(currentSessions: 10, targetSessions: 10, totalReceivedPackets: 10, tps: 1),
                CreateSample(currentSessions: 10, targetSessions: 10, totalReceivedPackets: 20, tps: 1),
                CreateSample(currentSessions: 10, targetSessions: 10, totalReceivedPackets: 30, tps: 1, disconnectCount: 1)
            ],
            []);
        var evaluator = new LoadValidationEvaluator();
        ...
        Assert.IsFalse(summary.Passed);
        Assert.IsTrue(summary.Failures.Any(failure => failure.Contains("Final disconnect count", StringComparison.Ordinal)));
    }

    [TestMethod]
    public void LoadValidationEvaluator_FailsReceiveTimeoutClass()
    {
        LoadValidationStage stage = CreateStage(targetSessions: 10);
        JsonlReadResult readResult = new(
            [
                ...
                CreateSample(
                    currentSessions: 10,
                    targetSessions: 10,
                    totalReceivedPackets: 30,
                    tps: 1,
                    socketErrorCountsByClass: new Dictionary<string, long>
                    {
                        [LoadValidationThresholds.ReceiveTimedOutClass] = 1
                    })
            ],
            []);
        var evaluator = new LoadValidationEvaluator();
        ...
        Assert.IsFalse(summary.Passed);
        Assert.IsTrue(summary.Failures.Any(failure => failure.Contains(LoadValidationThresholds.ReceiveTimedOutClass, StringComparison.Ordinal)));
    }
